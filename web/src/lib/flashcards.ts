...
export type Flashcard = {
   ...
   * the card. Optional — the key is omitted entirely when there is none.
   */
  sentence?: string;
  /**
   * An English translation of {@link sentence}, revealed below it behind the
   * card's eye toggle. Optional — omitted entirely when there is none.
   */
...
export function createCard(
  ...
  const translation = sentenceTranslation?.trim();
  if (translation) card.sentenceTranslation = translation;
  return card;
}
...
type ColumnLayout = {
  ...
  /** -1 when there is no sentence column. */
  sentence: number;
  /** -1 when there is no translation column. */
  translation: number;
 ...
 * collection, sentence, translation.
 */
function resolveLayout(rows: string[][]): ColumnLayout {
  ...
  if (hasHeader) {
    return {
      ...
      sentence: header.indexOf("sentence"),
 ...
 * positionally as japanese, english, pronunciation, collection, sentence,
 * translation. Rows missing a Japanese or English value are skipped and
 * reported; a blank/absent collection falls back to {@link DEFAULT_COLLECTION}.
 * An optional `pronunciation` column (read third when there is no header)
 * carries an English reading of the Japanese word, an optional `sentence`
 * column (read fifth) carries a basic Japanese example sentence, and an
 * optional `translation` column (read sixth) carries an English translation of
 * that sentence; each is left off the card when blank or absent.
 */
export function parseFlashcardsCsv(text: string): ParseResult {
  ...
  layout.dataRows.forEach((row, index) => {
    ...
    const sentence = (
      layout.sentence >= 0 ? row[layout.sentence] ?? "" : ""
    ).trim();
    const translation = (
      layout.translation >= 0 ? row[layout.translation] ?? "" : ""
    ).trim();
 ...
 * read again. Emits a
 * `japanese,english,pronunciation,collection,sentence,translation` header
 * followed by one row per card, so a deck round-trips through export and
 * re-upload unchanged. Cards without a value for an optional column write an
 * empty cell there. Papa handles quoting of values containing commas, quotes,
 * or newlines.
 */
