...
- **Workflow**: At the start of a task, pull the latest `dev`. After completing an implementation, commit and open a pull request to `dev`. The PR is gated by a pre-merge CI check (linting, tests, and build) that runs automatically; wait for it to pass, then **merge the PR yourself** (`gh pr merge <num> --merge`) — CI/CD does NOT auto-merge. To check a CI/CD workflow's status, watch it with `gh pr checks <workflow> --watch --interval 15` instead of polling repeatedly. If a workflow never starts, or gets canceled for no apparent reason, a merge conflict with `dev` is the most likely cause — check the PR for conflicts and resolve them before assuming the pipeline is broken. Resolve any merge conflicts first. After merging, sync your local `dev` with the remote in the **main project directory** (where `dev` is checked out), not in a worktree — git won't update a branch checked out elsewhere. A slower build/deploy runs on `dev` after the merge; you don't need to wait for it — if it goes red, fix forward on `dev`.
