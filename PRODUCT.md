...
A personal Japanese-learning flashcards app. The user uploads a CSV of words and the app turns each row into a flip-and-shuffle flashcard (Japanese on one side, English meaning on the other), grouped by the folder/category named in the file. Folders can be given colored tags, and the deck screen can filter to the folders sharing a tag. The deck and its organization are saved to the cloud (Firestore) between visits, and the user can export their cards back to CSV. Success is: drop in a file, and within seconds be studying clean, correctly-parsed cards with zero ceremony — then find them right where you left them next time, on any device.
